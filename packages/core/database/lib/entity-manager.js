        ...
        if (attribute.joinTable) {
          ...
          if (isOneToAny(attribute) && isBidirectional(attribute)) {
            await this.createQueryBuilder(joinTable.name)
              .delete()
              .where({ [inverseJoinColumn.name]: _.castArray(data[attributeName]) }) // TODO: would break with connect
              .where(joinTable.on || {})
              .execute();
          }
          ...
          const relationsToAdd = _.uniqBy('id', assocs.connect || assocs);
      ...
      for (const attributeName of Object.keys(attributes)) {
        ...
        if (attribute.joinTable) {
          ...
          if (onlyDeleteRelation) {
            ...
            if (isPartialUpdate) {
              const idsToRemove = _.map('id', connect.concat(disconnect));
              ...
              if (connect.length) {
                ...
                insert = _.uniqBy('id', connect).map((rel, idx) => ({
                  [joinColumn.name]: id,
